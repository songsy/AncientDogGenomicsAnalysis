...
# deal with each chromosome, use run-by-id.pl
...
def read_locus_file_by_chr(file,SEQ,sample_list):            # locus interval is bed format, zero based, transform into 1-based
	locus={}
	f=open(file,"r")
	for line in f:
		line = line.strip().split("\t")
		...
		if chr not in locus.keys():
			locus[chr]=[]
		locus[chr].append([chr,int(pos1)+1,int(pos2),"_".join(line)]) 
	return locus,SEQ
...
def process_each_chrom(locus,SEQ,ref,sample_name,index):
	matrix_all=[]
	size_all=[]
	chr=sorted(locus.keys())[index]
	print chr
	f=open('Gphocs_input_canfam3.1_all_sample_GQ30_BQ5_MQ15_DP5_unrecal_aDNA_%s_rm_damage.txt' %(chr),"w")
	for loci in locus[chr]:
		matrix=np.zeros((len(sample_name),len(sample_name)))
		size=np.zeros((len(sample_name),len(sample_name)))
		status=1
		...
		pos2=int(loci[2])
		tag=loci[3]
		ref_seq=ref.fetch(chrom,pos1-1,pos2)
		ref_seq=ref_seq.upper()
		record=SEQ[tag]
		record=remove_CG(record)
		print tag
		...
		if status:
			print >>f,"%s.%s-%s\t%i\t%s" %(chr,pos1-1,pos2,len(sample_name),pos2-pos1+1)
			for j in range(len(sample_name)):
				print >>f,"%s\t%s" %(sample_name[j],record[j])
			f.write("\n")
			matrix_all.append(matrix)
			size_all.append(size)
	return (matrix_all,size_all)
...
def process_each_chrom_wrapper(args):
	args2 = args[0] + (args[1],)
	stats=process_each_chrom(*args2)
	return stats

def calc_NJ(dist,size,sample_name):
	f_matrix=open("dist_matrix_canfam3.1_all.txt","w")
	matrix=np.zeros((len(sample_name),len(sample_name)))
...
def aln_to_fasta_bootstrap(dist,size,status,sample_name):
	...
	searchval = 1
	usable=list(range(len(dist)))
	for index in range(Nbootstrap):
		print 'bootstrap:',index
		f_matrix=open("dist_matrix_canfam3.1_bootstrap_%s.txt" %(index),"w")
		choose=np.random.choice(usable,size=len(dist))
		matrix=np.zeros((len(sample_name),len(sample_name)))
		length=np.zeros((len(sample_name),len(sample_name)))
		for i in choose:
			matrix+=dist[i]
...
if __name__=="__main__":
	...
	parser.add_argument("--chr",default=0,type=int,dest='chr',help="chr index")
	...
	locus,SEQ=read_locus_file_by_chr(locus_file,SEQ,sample_list)  

	print sorted(locus.keys())
#	print 12
...
#	print m[0],m[1],m[2]
	...
	m=process_each_chrom(locus,SEQ,ref,sample_list,args.chr)
...
#	print m[1][1]
	...
	result1=pool.map(process_each_chrom_wrapper,itertools.izip(itertools.repeat((locus,SEQ,ref,sample_list)),range(len(locus))))
	pool.close()
	pool.join()
