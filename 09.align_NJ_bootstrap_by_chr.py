...
def remove_damage(record,ref_seq):
	...
	for seq in record:
		for j in range(len(seq)):
			if (ref_seq=="C" and "T" in annotation[seq[j]]) or (ref_seq=="T" and "C" in annotation[seq[j]]):
				pos.append(j)
			elif (ref_seq=="G" and "A" in annotation[seq[j]]) or (ref_seq=="A" and "G" in annotation[seq[j]]):
				pos.append(j)
	for i in range(len(record)):
		old_seq=record[i]
		seq = ""
...
def process_each_chrom(locus,SEQ,ref,sample_name,index):
	...
	print chr
	f=open('Gphocs_input_canfam3.1_all_sample_GQ30_BQ5_MQ15_DP5_unrecal_aDNA_%s.txt' %(chr),"w")
	for loci in locus[chr]:
		matrix=np.zeros((len(sample_name),len(sample_name)))
		size=np.zeros((len(sample_name),len(sample_name)))
		status=1
...
#	m=process_each_chrom(locus,SEQ,ref,sample_list,31)
