...
###This program calls genotypes from bam files while taking into account post mortem damage as estimate by MapDamage.
...
filenamein=argv[1]#'/vault/veeramah/dog/processed/run1_2/merged_all_run1_run2.MKdup.RG.sort.realign.bam'#argv[1]
filenameinB=argv[2]#'/vault/veeramah/dog/bed_files/neutralLoci-geneFlank10k-sep30k.bed'#argv[2]
ref_file=argv[3]#'/vault/public/dog/canfam3.fa'#argv[3]
mapdamageCT=argv[4]#'/vault/veeramah/dog/processed/run1_2/mapDamage_res/mapDamage_res_rand0.1_A/5pCtoT_freq.txt'#argv[4]
mapdamageGA=argv[5]#'/vault/veeramah/dog/processed/run1_2/mapDamage_res/mapDamage_res_rand0.1_A/3pGtoA_freq.txt'#argv[5]
filenameout=argv[6]
min_RD=5 #int(argv[3])
MQ=15 #float(argv[4])
BQ=5 #float(argv[5])
...
plotfile=filenameout+'.aDNA.expMDfit_WB.pdf'
MDfile=filenameout+'.aDNA.fitted_model_params_WB'
filenameout1=filenameout+'.aDNA.emit_all.vcf'
filenameout2=filenameout+'.aDNA.vcf'
filenameout3=filenameout+'.aDNA.haploid.emit_all.vcf'
...
out1='##Custom variant caller for_aDNA, emit all diplod\n'
out2='##Custom variant caller for_aDNA, variable diploid\n'
out3='##Custom variant caller for_aDNA, emit all haploid\n'
    ...
    for ggg in range(len(GLs)):
        ...
        if SEG[ggg]<>-9:   ###this sites was callable, even if QUAL was low
            ...
            if SEG[ggg]==0:  #site is homozygous reference, just note, GT,DP and GQ
                out=out+'\tGT:DP:GQ\t0/0:'+str(RDs[ggg][REFs[ggg]])+':'
                if GQs[ggg]>99:
                    out=out+'99\n'
                else:
