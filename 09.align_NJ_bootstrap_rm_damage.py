...
def calc_distance_for_each_loci(locus,SEQ,ref,sample_name,index):
	matrix=np.zeros((len(sample_name),len(sample_name)))
	size=np.zeros((len(sample_name),len(sample_name)))
	status=1
...
#	print 12
	...
	'''
	m=calc_distance_for_each_loci(locus,SEQ,ref,sample_list,0)
	print m[0],m[1],m[2]

	pool = multiprocessing.Pool(processes=20)
	result1=pool.map(calc_distance_for_each_loci_wrapper,itertools.izip(itertools.repeat((locus,SEQ,ref,sample_list)),range(len(locus))))
	pool.close()
	pool.join()
	...
	for i in range(len(locus)):
		m=calc_distance_for_each_loci(locus,SEQ,ref,sample_list,i)
		dist.append(m[0])
		size.append(m[1])
		status.append(m[2])
	print sum(status)
